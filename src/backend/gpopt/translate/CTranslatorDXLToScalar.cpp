	...
	List **param_ids)
{
	...
	if (EdxlopScalarIdent == inner_child_node->GetOperator()->GetDXLOperator())
	{
		// Ident
		inner_ident =
			CDXLScalarIdent::Cast(inner_child_node->GetOperator());
		inner_expr = (Expr *) param;
	}
	else if (EdxlopScalarCast == inner_child_node->GetOperator()->GetDXLOperator() &&
		inner_child_node->Arity() > 0 &&
		EdxlopScalarIdent == (*inner_child_node)[0]->GetOperator()->GetDXLOperator())
	{
		// Casted Ident
	...
	args = gpdb::LAppend(args, (Expr *) inner_expr);
...
Expr *
CTranslatorDXLToScalar::TranslateDXLCoerceViaIOWithChildExpr(
	...
	const CDXLNode *scalar_coerce_node, CMappingColIdVar *colid_var)
{
	...
	return (Expr *) TranslateDXLCoerceViaIOWithChildExpr(dxlop, child_expr);
}
